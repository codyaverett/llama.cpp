#!/usr/bin/env bash
# Launch llama-server for <NEW MODEL NAME> split across the Vega cards.
...
MODEL="$MODELS_DIR/CHANGE-ME.gguf"
...
NP="${LLAMA_PARALLEL:-4}"
echo "Starting <NEW MODEL> llama-server [$BACKEND] on $HOST:$PORT (ctx=$CTX, parallel=$NP, FA on${LLAMA_CACHE_TYPE:+, kv=$LLAMA_CACHE_TYPE}), 9x Vega layer-split, no-mmap" >&2
